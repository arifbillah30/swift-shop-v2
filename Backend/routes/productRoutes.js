...
// helper to coerce/whitelist enums
const enumOrNull = (val, allowed) => (val && allowed.includes(val) ? val : null);

// ==============================
// Admin: List orders (summary)
// GET /admin/orders?limit=50&offset=0&status=pending&user_email=foo@bar.com
// ==============================
router.get('/admin/orders', async (req, res) => {
  try {
    ...
    const userEmail = req.query.user_email || null;
    ...
    // GROUP BY–safe aggregation via subquery
    const sql = `
      SELECT
        o.id,
        o.order_number,
...
router.get('/admin/orders/:id', async (req, res) => {
  ...
  try {
    ...
    // Return the snapshot field under its original column name to avoid UI alias drift
    const itemsSql = `
      SELECT
        oi.id,
        oi.order_id,
        oi.product_id,
        oi.variant_id,
        oi.product_name_snapshot,     -- <— keep original column name
        oi.unit_price,
        oi.quantity,
        oi.line_total,
...
router.put('/admin/orders/:id/cancel', async (req, res) => {
  ...
  try {
    const [result] = await db.query(
      `UPDATE orders
       SET status='cancelled', fulfillment_status='unfulfilled', updated_at=NOW()
       WHERE id=?`,
      [orderId]
    );
    if (result.affectedRows === 0) {
