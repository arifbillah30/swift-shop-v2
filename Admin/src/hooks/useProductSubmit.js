...
const useProductSubmit = (id) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Check for images - either imageUrl array or pending files
      const hasImages = (Array.isArray(imageUrl) && imageUrl.length > 0) || 
                       imageUrl || 
      ...
      // Temporarily make category optional for testing
      ...
      const productData = {
        ...
        category: defaultCategory[0]?._id || null,
      ...
      if (updatedId) {
        ...
        if (isCombination) {
          ...
          setTag(Array.isArray(res.tag) ? res.tag : JSON.parse(res.tag || '[]'));
