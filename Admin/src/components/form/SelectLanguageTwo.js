...
const SelectLanguageTwo = ({ handleSelectLanguage, register }) => {
  ...
  // If there's an error or no data, provide fallback options
  const fallbackLanguages = [
    { _id: 'en', iso_code: 'en' },
    { _id: 'de', iso_code: 'de' }
  ];

  const languageOptions = error || !data ? fallbackLanguages : data;
  ...
  return (
    <>
      <select
        ...
        onChange={(e) => handleSelectLanguage && handleSelectLanguage(e.target.value)}
