...
const ShowHideButton = ({ id, status, category, currencyStatusName }) => {
  ...
  const handleChangeStatus = async (id) => {
    ...
    try {
      ...
      // Handle different status formats based on the context
      if (location.pathname === "/products") {
        // For products, map database statuses to show/hide
        if (status === "active" || status === "show") {
          newStatus = "hide"; // This will be mapped to 'draft' in backend
        } else {
          newStatus = "show"; // This will be mapped to 'active' in backend
        }
      } else {
  ...
  // Determine if the button should be checked based on status
  const isChecked = () => {
    if (location.pathname === "/products") {
      // For products, 'active' status means published/shown
      return status === "active" || status === "show";
    } else {
      // For other components, use the original logic
      return status === "show";
    }
